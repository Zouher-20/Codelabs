...
const UserProfile = async ({ params }: { params: { 'id': string } }) => {
    ...
    const tabs = [
        { name: 'Personal informatio', component: <PersonalInfo user={user as unknown as userType} /> },
    ];

    return <div className="flex flex-col gap-4 px-2">
        <h1 className=" text-2xl capitalize">{user?.username} Profile</h1>
        <Avatar user={{ username: user?.username as string, userimage: user?.userImage as string }} />
