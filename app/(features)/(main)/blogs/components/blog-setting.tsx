...
const BlogSetting = ({ blogID }: { blogID: string }) => {
    ...
    const ToggleModal = () => {
        if (document) {
            (document.getElementById('delete-blog-modal') as HTMLFormElement)?.showModal();
        }
    }
    return <div className="dropdown dropdown-end">
        ...
        <ul tabIndex={0} className="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow">
            <li><Link href={`/blogs/${blogID}`} className="flex">
        ...
        <DeleteBlogModal onDelete={() => { HandleDelete(blogID) }} />
    </div>
}
