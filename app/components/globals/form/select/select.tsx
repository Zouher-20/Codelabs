...
const SelectField = ({ placeholder, field, form, options, isMulti, errors }: type.CustomSelectProps) => {
    ...
    return (
        isMounted ? (
            <div className="flex flex-col gap-1 ">
                <label className={"input input-bordered max-w-sm flex h-fit items-center gap-2  pr-0 " + (errors ? ' input-error' : '')}>
...
const selectStyle = (errors: string | null) => {
    const style: StylesConfig = {
        ...
        option: (baseStyles, state) => ({
            ...baseStyles,
            backgroundColor:
                state.isDisabled ? undefined
                    : state.isSelected ? '#1D231C'
                        : state.isFocused ? '#1D231C' : undefined,
            ':active': {
                ...baseStyles[':active'],
